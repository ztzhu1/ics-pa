...
static def_DHelper(J) {
  id_src1->imm = 0;
  sword_t simm = s->isa.instr.j.imm9_0 | s->isa.instr.j.imm10_10 << 10 | s->isa.instr.j.imm18_11 << 11 | s->isa.instr.j.simm19_19 << 19;
...
def_THelper(main) {
  // every line below executes (if matches)
  // {
  //  decode_id(s, width);
  //  return table_tab(s);
  // }
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00000 11", I     , load);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , alg);
