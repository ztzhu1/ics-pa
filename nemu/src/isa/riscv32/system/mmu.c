...
paddr_t isa_mmu_translate(vaddr_t vaddr, int len, int type) {
  ...
  // page table 2's PPN
  uintptr_t pte1_ptr = (uintptr_t)(csr_state.satp.ppn * PAGE_SIZE + VA_VPN1(vaddr) * PTE_SIZE);
  ...
  uintptr_t pte2_ptr = (uintptr_t)(PTE_PPN(pte1) * PAGE_SIZE + VA_VPN0(vaddr) * PTE_SIZE);
